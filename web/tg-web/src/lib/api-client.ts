...
export interface TelegramStatus {
  ...
  /** Total raw events received from Telegram since monitoring started */
  eventsReceived: number;
  /** Events that matched at least one keyword */
  eventsMatched: number;
  /** Events whose chatId was not found in the groups map (ID mismatch diagnostic) */
  eventsMissedGroup: number;
}
...
export const getGetAuthStatusQueryKey = () => ['auth', 'status'] as const;
...
export function useGetAuthStatus(
  options?: { query?: Partial<UseQueryOptions<AuthStatus>> },
) {
...
export function useAppLogout(
  ...
  return useMutation({
    mutationFn: () =>
      apiFetch('/api/auth/logout', { method: 'POST', body: '{}' }),
    ...options,
  });
}
